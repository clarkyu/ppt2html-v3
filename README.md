...
- **懂你 + 逐环节大纲** —— AI 先问 1~2 个关键问题（选模型时已**预取**，几乎秒开，可跳过），再用一句「**我的理解**」还原你的意图供核对；随后**先确认整体结构**（分几个部分、**每部分多少页 / 多长时间**一目了然、可调），再**逐个环节流式细化**每一页（封面 → 各部分 → 结束 → 整份总览，一步步确认，不一次性硬生成）；细化与最终成稿都**实时显示进度**（逐页标题即时出现，不是干转圈）。
