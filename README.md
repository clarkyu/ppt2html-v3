...
- **懂你 + 分步大纲** —— AI 先问 1~2 个关键问题（选模型时已**预取**，几乎秒开，可跳过），再用一句「**我的理解**」还原你的意图供核对；随后**分两步确认大纲**——先看分成哪几个**部分**，确认后再把每个部分**细化成逐页大纲**（每页带序号与彩色版式标签，点文字改标题/要点、↑↓ 调序、增删页），最后才生成完整课件。少打扰、也把关键信息问全、问准。
- **多模型预设 + 下拉选型** —— 设置里一键切换 DeepSeek / 通义千问 / Kimi / OpenAI / Claude，模型从**下拉菜单**里选（也可选「自定义…」自行填写），填好各自 API Key 即可。
