...
- **开箱即用 · 系统兜底** —— 默认使用**系统提供的 DeepSeek（deepseek-v4-pro，默认开启思考/推理）**，**免填 API Key** 即可直接生成，所有用户无痛上手；想用 Claude / OpenAI / Gemini 等其它模型时再填自己的 Key。生图模型需自备 Key，未提供时只生成文本型课件。（注：静态站点内置的系统 Key 会随公开构建产物一起下发、可被读取，属于「换取零门槛」的取舍。）
...
- **懂你 + 逐环节大纲** —— AI 先问 1~2 个关键问题（选模型时已**预取**，几乎秒开，可跳过），再用一句「**我的理解**」还原你的意图供核对；随后**先确认整体结构**（分几个部分、**每部分多少页 / 多长时间**一目了然、可调），再**逐个环节流式细化**每一页（封面 → 各部分 → 结束 → 整份总览，一步步确认，不一次性硬生成）；**总览按环节分类**（每个环节可**折叠**、可**增删部分**、可在环节内加页），并可点「上一步」或某环节「去修改」**回退修改**；细化与最终成稿都**实时显示进度**（逐页标题即时出现，不是干转圈）。
...
- **每页自动配背景图** —— 生成时按页面内容**联网搜图**，为每一页配一张**贴合主题、淡淡的**背景图（有主题色蒙版压住，不抢内容）。默认用免费的 **Openverse** 图库（CC 授权、**无需 Key**、开箱即用）；填了自己的 **Unsplash / Pexels** Key 则优先用它（画质更好）。可在「设置」里一键关闭，或在编辑器里逐页**换图 / 移除**。
- **不溢出** —— 生成时约束每页信息量，渲染时再**自动缩放**内容以适配 16:9 画面，任何页面都完整可见、绝不裁切（对已有课件同样生效）。
- **精美主题** —— 内置 5 套设计主题（极光 / 水墨 / 暖阳 / 森林 / 深邃），排版、配色、渐变一体成型。
- **专业播放器** —— 基于 [reveal.js]：全屏、键盘 / 触屏翻页、过场动画、逐条渐入、总览模式、演讲者备注、打印导出 PDF。
- **标题自适应排版** —— 每页标题 / 副标题过长时**自动缩小字号、平衡断行**并防止溢出（长中文标题也能塞进版面）；播放与编辑预览里都实时生效。
