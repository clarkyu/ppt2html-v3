...
const DICT: Record<string, Entry> = {
  ...
  'style.button': { zh: '换风格（主题一键换装）', en: 'Restyle (swap the whole look)' },
  'style.title': { zh: '换一套风格', en: 'Pick a style' },
  'style.hint': {
    zh: '一键换装：配色、字体气质、抽象背景全套跟随，内容不变。',
    en: 'One click swaps palette, type flavor and abstract backgrounds — content untouched.',
  },
  ...
  'style.bg': { zh: '背景色', en: 'Background' },
  'style.accent': { zh: '强调色', en: 'Accent' },
  'style.accent2': { zh: '次强调色', en: 'Accent 2' },
  'style.serif': { zh: '衬线标题（更书卷气）', en: 'Serif titles (bookish)' },
  ...
  'share.hint': {
    zh: '课件已压缩进链接本身——不经过任何服务器。对方打开即可播放并可保存副本；配图背景会在对方设备上自动补齐。',
    en: 'The deck is compressed into the link itself — no server involved. Openers can play it and keep a copy; backgrounds re-fill on their device.',
  },
  ...
  'share.unsupported': {
    zh: '当前浏览器不支持压缩 API，无法生成分享链接。',
    en: 'This browser lacks the compression API — share links are unavailable.',
  },
  ...
  'rw.button': { zh: '用 AI 改写当前页（保持版式）', en: 'AI-rewrite this slide (keeps the layout)' },
  'rw.title': { zh: 'AI 改写第 {n} 页', en: 'AI rewrite — slide {n}' },
  'rw.hint': {
    zh: '说一句想怎么改，AI 会保持版式重写这一页的内容，改完立即生效、可撤销。',
    en: 'Say how to change it — AI rewrites this slide keeping the layout; applies live, undoable.',
  },
  'rw.placeholder': {
    zh: '例如：更口语化 / 补一个真实案例 / 精简到三条要点',
    en: 'e.g. friendlier tone / add a real example / cut to three bullets',
  },
  'rw.go': { zh: '改写', en: 'Rewrite' },
  'rw.again': { zh: '再改一轮', en: 'Rewrite again' },
  'rw.busy': { zh: '正在改写…', en: 'Rewriting…' },
  'rw.done': { zh: '已改写并保存。', en: 'Rewritten and saved.' },
  'rw.undo': { zh: '撤销本次改写', en: 'Undo this rewrite' },
  'rw.undone': { zh: '已撤销。', en: 'Reverted.' },
  'refine.button': { zh: '一键精修（按质量硬指标扫描全篇，只重写有问题的页）', en: 'Refine pass (scan for quality issues, rewrite only flagged pages)' },
  'refine.title': { zh: '一键精修', en: 'Refine pass' },
  'refine.found': {
    zh: '按质量硬指标扫描：{n} 页共 {m} 处可修问题（容量超限 / 缺具体锚点 / 标签式要点 / 备注缺失）。AI 将只重写这些页，其余不动，可整体撤销。',
    en: 'Mechanical scan: {m} issue(s) on {n} page(s) — capacity overruns, missing anchors, label-like bullets, missing notes. Only flagged pages get rewritten; fully undoable.',
  },
  'refine.clean': { zh: '全篇通过质量硬指标检查，没有需要精修的页 ✓', en: 'Every page passes the mechanical checks — nothing to refine ✓' },
  ...
  'ge.hint': {
    zh: '说一个全篇性的修改要求，AI 先给出「哪些页要改、各自怎么改」的计划，你确认后才逐页执行，可整体撤销。',
    en: 'Give one deck-wide instruction. AI proposes a per-page plan first; nothing is rewritten until you confirm. Fully undoable.',
  },
  'ge.placeholder': {
    zh: '例如：整体更口语化 / 每页补一个真实例子 / 把「用户」统一改成「客户」',
    en: 'e.g. friendlier tone throughout / add a real example to each page / rename “users” to “customers” everywhere',
  },
  'ge.planBtn': { zh: '生成修改计划', en: 'Draft the plan' },
  'ge.replan': { zh: '重新计划', en: 'Re-plan' },
  'ge.planning': { zh: '正在通读全篇、规划修改…', en: 'Reading the deck and drafting a plan…' },
  'ge.planReady': { zh: '确认计划后开始执行；也可修改指令重新计划。', en: 'Confirm to apply, or tweak the instruction and re-plan.' },
  'ge.noOps': { zh: 'AI 认为没有页面需要为这个要求修改——试试更具体的指令。', en: 'The plan came back empty — try a more specific instruction.' },
  'ge.run': { zh: '执行 {n} 页修改', en: 'Apply to {n} page(s)' },
  ...
  'settings.imageGen': { zh: 'AI 配图（图像生成，可选）', en: 'AI illustrations (image generation, optional)' },
  'settings.imageGenHint': {
    zh: '配置任意 OpenAI 兼容的图像生成接口（OpenAI DALL·E、SiliconFlow 等），编辑器里即可为每页生成定制插图。费用走你自己的 API。',
    en: 'Point at any OpenAI-compatible image endpoint (OpenAI DALL·E, SiliconFlow…) to generate per-page illustrations from the editor. Billed to your own API.',
  },
  'settings.imageGenBase': { zh: '接口地址,如 https://api.openai.com/v1', en: 'Base URL, e.g. https://api.openai.com/v1' },
  'settings.imageGenKey': { zh: 'API Key', en: 'API key' },
  'settings.imageGenModel': { zh: '模型,如 dall-e-3', en: 'Model, e.g. dall-e-3' },
  ...
  'ed.genImg': { zh: 'AI 配图', en: 'AI illustration' },
  'ed.genImgTitle': {
    zh: '用图像生成模型为本页定制一张背景插图（需在设置中配置图像生成 API）',
    en: 'Generate a custom background illustration for this page (configure an image API in Settings)',
  },
  'ed.genImgNoKey': {
    zh: '尚未配置图像生成 API——请到「设置 → AI 配图」填写 OpenAI 兼容的图像接口与密钥。',
    en: 'No image-generation API configured — add an OpenAI-compatible endpoint and key in Settings → AI illustrations.',
  },
  'ed.genImgStart': { zh: '正在生成配图…通常需要 10~30 秒', en: 'Generating the illustration… usually 10–30s' },
  'ed.genImgDone': { zh: 'AI 配图已应用到本页。', en: 'AI illustration applied to this page.' },
  'ed.genImgFailed': { zh: 'AI 配图生成失败', en: 'Image generation failed' },
